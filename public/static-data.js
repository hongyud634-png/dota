(function () {
  const CONFIG = {
    ...
    storageKey: "opendota_static_user",
  };
    ...
    for (let attempt = 1; attempt <= retries; attempt += 1) {
      ...
      try {
        ...
        if (response.status === 429 || response.status >= 500) {
          ...
          await sleep(Math.min(5000, 500 * attempt ** 2));
          continue;
        }
  ...
  function dataUrl() {
    return new URL("data/opendota_league_18113_seed.json", document.baseURI).toString();
  }
  ...
  function loadSeed(seedData) {
    ...
    state.lastSyncFinishedAt = seedData.generatedAt ? `${seedData.generatedAt} seed` : isoBeijingNow();
  }

  async function init() {
    if (state.initialized) return;
    if (!state.initPromise) {
      state.initPromise = fetchJson(dataUrl(), { retries: 2, timeoutMs: 20000 }).then((seedData) => {
        loadSeed(seedData);
  ...
  async function syncLatest() {
    ...
    try {
      ...
      state.lastError = `静态模式同步失败：${error?.message ?? String(error)}`;
    } finally {
  ...
  function currentUser() {
    try {
      const user = JSON.parse(localStorage.getItem(CONFIG.storageKey) || "null");
  ...
  async function dashboard(options = {}) {
    await init();
    if (options.refresh) await syncLatest();
    return buildDataset(options);
  }
